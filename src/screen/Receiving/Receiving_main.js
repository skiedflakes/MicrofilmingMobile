...
export default function Receiving_main({
  navigation: {goBack},
  navigation,
  route,
}) {
  ...
  const get_deliveries_data = () => {
    ...
    fetch(global.global_url + '/receiving_stocks/get_receiving_data.php', {
      method: 'POST',
      headers: {
      ...
      .then((responseJson) => {
        ...
        var data = responseJson.array_data.map(function (item, index) {
          return {
            dr_header_id: item.rr_header_id,
            ...
            <View style={styles.main_modalView}>
              ...
              <View style={{flexDirection: 'row', padding: 2, marginTop: 10}}>
                <TouchableOpacity
                  onPress={() => {
                    setmodal_main_Visible(false);
                    navigation.navigate('Upload Receiving', {
    ...
    <TouchableOpacity
      ...
      <View style={styles.item}>
        <View style={{flex: 3, alignItems: 'center'}}>
          ...
          <Text style={styles.text_inv}>Purchase #: {invoice_no}</Text>
        </View>
