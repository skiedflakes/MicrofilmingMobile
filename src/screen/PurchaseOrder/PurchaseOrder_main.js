...
export default function PurchaseOrder_main({
  navigation: {goBack},
  navigation,
  route,
}) {
  ...
  const get_deliveries_data = () => {
    ...
    fetch(global.global_url + '/purchase_order/get_po_data.php', {
      method: 'POST',
      headers: {
      ...
      .then((responseJson) => {
        ...
        var data = responseJson.array_data.map(function (item, index) {
          return {
            dr_header_id: item.po_header_id,
            delivery_number: item.po_number,
            query_data: item.po_number,
          };
        });
  ...
  const get_images_data = (reference_number) => {
    ...
    fetch(global.global_url + '/receiving_stocks/get_micro_filming_img.php', {
      method: 'POST',
      headers: {
        Accept: 'application/json',
        'Content-Type': 'multipart/form-data',
            ...
            <View style={styles.main_modalView}>
              ...
              <View style={{flexDirection: 'row', padding: 2, marginTop: 10}}>
                <TouchableOpacity
                  onPress={() => {
                    setmodal_main_Visible(false);
                    navigation.navigate('Purchase Order Upload', {
